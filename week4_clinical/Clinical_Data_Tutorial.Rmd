...
Which side do we place it on? Let's think. If we place the mask in the rows side, R will travel down the rows of the clinical data frame and check the corresponding TRUE/FALSE value, keeping or ignoring rows accordingly. If we place the mask in the columns side, R will keep or ignore columns accordingly. HINT: draw out the data frame if this is confusing!
...
Before we get started, install and load the "survival" and "survminer" packages. Also install/load ggplot2 if it is not already on in your RStudio.
...
Let's create a column in clinical called survival_time. For some patients, we can find survival time using their days_to_death info. But for others, namely those who never had a registered death event, days_to_death = NA, and our best estimate becomes days_to_last_follow_up, since that is the longest definitive time we can say they survived. Using the days_to_death and days_to_last_follow_up columns, create a your survival_time column in clinical.
...
Great job so far! In this tutorial you explored the clinical data frame, you practiced boolean indexing, and you created a Kaplan-Meier plot.
